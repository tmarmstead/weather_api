...
function historyButtons(cityName) {
  let newStorageButton = $('<button>').attr('class', 'button ml-2 past').text(cityName);
  $('.local-storage-buttons').append(newStorageButton);
}

function localStorageCity(cityName) {
  let history = JSON.parse(window.localStorage.getItem('history')) || [];
  console.log("HISTORY FROM LOCAL STORAGE FUNCTION!!!!!!!", history);
...
// Populate Forecast when Page is opened with Last Searched City info
function populatePreviousForecast() {
  let lastSearched = $(".past").last().text();
  console.log("LAST SEARCHED LOG", lastSearched);

  function displayForecastOnRefresh(city) {
  ...
  displayForecastOnRefresh(lastSearched);
}

populatePreviousForecast();

// Loop Through all ".past" buttons to see if any are already present.
// Only append button if there are no duplicates

// $(".past").each(function () {
//   let buttonText = $(".past").text() + " ";
//   let buttonTextArray = [];

//   buttonTextArray.push(buttonText, ", ");

//   // console.log(buttonText.join(", "), "BUTTON TEXTTTTTTTTT")

//   console.log(buttonTextArray, "BUTTON TXT ARRAYYYYYY!!!!")
